...
- **README config/logging docs.** Every extension README should document configuration and logging behavior. If an extension has no user-facing configuration or retained logs, say so explicitly. When an extension has two or more user-facing settings fields, document them in a scannable table with `Field`, `Default`, and `Description` columns. Document environment variables separately from settings, using a `Variable`, `Default`, and `Description` table when there are two or more. If it writes logs or temp output, document location, retention/deletion behavior, and whether raw tool/process output may be included.
