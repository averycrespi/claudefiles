...
**Sandbox subsystem** — Manages a Lima VM for autonomous plan execution. `cco box create` provisions the VM, `cco box start`/`stop` control its lifecycle, and `cco box shell` opens an interactive session. The lima package is a thin wrapper around `limactl`.
