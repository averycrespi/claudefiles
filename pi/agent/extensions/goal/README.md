...
- `/goal-show` — show the current goal or report that none is set.
...
When `showWidget` is enabled and a goal exists, a compact widget appears above the editor. It shows the goal status and truncated objective. Completed goals also show one truncated evidence line.
...
The extension writes no retained logs or diagnostic files. Goal objectives and completion evidence are persisted in Pi session history as described above.

## Prior art and v1 omissions

This extension adapts the durable objective, lifecycle controls, model-visible goal context, and conservative completion audit from Codex-style goal workflows. v1 intentionally omits autonomous continuation, background scheduling, project-global goals, token/time accounting, budget-limited wrap-up behavior, and automatic TODO creation from goals.
